...
- **Formatting Modes & Safeguards:** Supports `Standard`, `Formal`, `Raw`, and `Debug` formatting. A customizable optional add-on ("Clean") removes filler words dynamically from any non-raw mode. The pipeline includes `RejectIfHallucinated` and `StripOutputWrappers` to protect against chat model hallucinations or layout prefixes (e.g., reverting to raw text if preservation ratio is low or markdown lists/bullet points are added).
